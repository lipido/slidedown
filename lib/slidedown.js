...
(function () {
  ...
  function renderArrows(slide) {
    ...
    slide.querySelectorAll('.sd-arrow, .arrow').forEach(function (arrow) {
      ...
      /* rect de un elemento en coordenadas de la slide */
      function rect(el) {
      ...
      function draw() {
        ...
        if (!fromSide) {
          var SAMPLES = 60;
          var pts = [];
          for (var i = 0; i <= SAMPLES; i++) pts.push(bez(a, c1, c2, b, i / SAMPLES));
          start = pts[0];
        ...
        if (!toSide) {
          var SAMPLES2 = 60;
          var pts2 = [];
          for (var j = 0; j <= SAMPLES2; j++) pts2.push(bez(a, c1, c2, b, j / SAMPLES2));
